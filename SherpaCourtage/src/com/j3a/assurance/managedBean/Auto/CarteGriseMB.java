...
public class CarteGriseMB implements Serializable {
		...
		private IdGenerateur idGenerateur;
		 ...
		 private List<SousCatVehicule> listSousCatVehicule=new ArrayList<SousCatVehicule>(); 
     ...
     /*   @PostConstruct
       public void postconstruct(){
			...
			try {
				...
				for (VehiculeRow F : getVehiculeList()) {
					...
					//	getObjectService().addObject(ApporteurVehi);
						...
						//mise � jour du vehicule et de la zone geographique
						
						vehi.setVehiculesAssures(Avn.getVehiculesAssures());
						
						getObjectService().updateObject(vehi);
						
						//R�cup du v�hi de la zone g�o
		...
		public List<SousCatVehicule> getListSousCatVehicule() {
			if (listSousCatVehicule.isEmpty()) {
				for (Object scat : getObjectService().getObjects("SousCatVehicule")) {
					listSousCatVehicule.add((SousCatVehicule) scat);
				}
			}
			
			return listSousCatVehicule;
		}

		public void setListSousCatVehicule(List<SousCatVehicule> listSousCatVehicule) {
			this.listSousCatVehicule = listSousCatVehicule;
		}
